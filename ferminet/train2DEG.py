...
def _add_suffix_before_ext(path: str, suffix: str) -> str:
  """Insert suffix (e.g., '.host0') before the file extension."""
...
def train(cfg: ml_collections.ConfigDict, writer_manager=None):
  ...
  # Convert mol config into array of atomic positions and charges              # switch to 2D
  atoms = jnp.stack([jnp.array(atom.coords[:2]) for atom in cfg.system.molecule])
  ...
  # START NETWORK SETUP -- MAKE FEATURE LAYER
  if cfg.network.make_feature_layer_fn:
    ...
    feature_layer = networks.make_ferminet_features(    #### If not set, networks.make_ferminet_features is used.
        natoms=charges.shape[0],
        nspins=cfg.system.electrons,
        ndim=cfg.system.ndim,
  ...
  # MAIN NETWORK -- FERMINET OR PSIFORMER
  ...
  if cfg.system.get('states', 0):
    if cfg.optim.objective == 'vmc_overlap':
      ...
      logabs_network = utils.select_output(
          networks.make_total_ansatz(signed_network,                 # make_total_ansatz -- look here
  ...
  # CHECKPOINTING (each 10 min, for example)
  ...
  if cfg.optim.objective == 'vmc':                              # OPTIMIZER OBJECTIVE
  ...
  def learning_rate_schedule(t_: jnp.ndarray) -> jnp.ndarray:      # LR SCHEDULE
    return cfg.optim.lr.rate * jnp.power(
        (1.0 / (1.0 + (t_/cfg.optim.lr.delay))), cfg.optim.lr.decay)



  # Construct and setup optimizer                     # OPTIMIZER
  ...
  if t_init == 0:
    logging.info('Burning in MCMC chain for %d steps', cfg.mcmc.burn_in)     # BURN IN MCMC
  ...
  if cfg.optim.optimizer == 'none' and opt_state_ckpt is not None:
    ...
    # training run.
    logging.info('No optimizer provided. Assuming inference run.')                  # NO OPTIMIZER MEANS INFERENCE RUN ?
    ...
    for t in range(t_init, cfg.optim.iterations):                     # MAIN TRAINING LOOP
      ...
      if save_walkers_now:
        ...
        if gather_hosts:
          ...
          prev_path = _add_suffix_before_ext(walkers_prev_path, suffix)  # e.g. walkers_prev.host0.npy
          last_path = _add_suffix_before_ext(walkers_last_path,  suffix) # e.g. walkers_last.host0.npy
          _atomic_save_npy(prev_path, prev_pos_host)
          _atomic_save_npy(last_path,  cur_pos_host)
